import { douyinAPI, douyinSign } from "./api";
import { logger, Networks } from "../model";
import {
  DouyinDataType,
  DouyinOptionsType,
  NetworksConfigType,
    ...
    switch (this.type) {
      ...
      case "评论数据": {
        ...
        while (fetchedComments.length < Number(data.number ?? 50)) {
          ...
          const requestCount = Math.min(
            Number(data.number ?? 50) - fetchedComments.length,
            maxPageSize
          );
        ...
        const finalResponse = {
          ...
          comments:
            data.number === 0
              ? []
              : fetchedComments.slice(0, Number(data.number ?? 50)),
      ...
      case "直播间信息数据": {
        ...
        if (UserInfoData.user.live_status !== 1)
          logger.error(UserInfoData.user.nickname + "未开启直播！");
        const room_data = JSON.parse(UserInfoData.user.room_data);
        this.URL = douyinAPI.直播间信息({
          room_id: UserInfoData.user.room_id_str as string,
          web_rid: room_data.owner.web_rid as string,
        });
      ...
      case "申请二维码数据": {
        this.URL = douyinAPI.申请二维码({
          verify_fp: data.verify_fp as string,
        });
